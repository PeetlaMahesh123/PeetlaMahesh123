# Hi 👋, I'm Peetla Mahesh

**Aspiring Software Developer | Full-Stack Developer | Java Developer**

A passionate and motivated software developer with strong foundations in full-stack development, problem-solving, and building real-world applications. I enjoy creating scalable, user-friendly applications and continuously improving my skills to match modern industry standards.

---

## 🚀 About Me

* 🎓 B.E. in Computer Science and Engineering
* 💻 Interested in Full-Stack Development and Software Engineering
* 🌱 Continuously learning and building real-world projects
* 🧠 Strong understanding of core computer science concepts
* 🎯 Focused on writing clean, efficient, and maintainable code

---

## 💻 Programming Languages

Java • Python • JavaScript • C • SQL

---

## 🌐 Web Development

HTML • CSS • React.js • Node.js (Basics) • Responsive Design • REST APIs

---

## 🗄️ Backend, Frameworks & Databases

* **Backend Technologies:** Java, JDBC
* **Frameworks:** **Hibernate ORM**, **Spring Boot (Basics)**
* **Databases:** MySQL • MongoDB
* CRUD Operations • Authentication & Authorization
* PHP (Basics) • XAMPP

---

## 🧠 Core CS Concepts

Data Structures & Algorithms • OOP
Time & Space Complexity • Searching & Sorting
Database Normalization

---

## 🛠️ Tools & Platforms

Git & GitHub • VS Code • Eclipse / IntelliJ
Postman • Chrome DevTools • Firebase

---

## 🎨 UI/UX

Figma (Basics) • Wireframing • Component-based UI Design

---

## 📌 Projects

### 🛒 Shop Management System (Hibernate)

**Technologies:** Java, Hibernate ORM, MySQL

* Developed a Hibernate-based Shop Management System with **admin-only authentication** using **enum-based role management**
* Used **Java `enum`** to define and enforce **ADMIN role access**, preventing unauthorized user operations
* Implemented full **CRUD operations** using Hibernate ORM
* Ensured **secure transaction management** with MySQL
* Strengthened understanding of ORM, role-based access control, and enterprise Java application design

---

### 🎓 Student Management System (JDBC)

**Technologies:** Java, JDBC, MySQL

* Developed a student management system using JDBC
* Implemented CRUD operations for student records
* Used JDBC for database connectivity and SQL query execution
* Strengthened understanding of Java backend logic and database handling

---

### 🏥 Hospital Management System (JDBC)

**Technologies:** Java, JDBC, MySQL

* Built a hospital management system to manage patients, doctors, and appointments
* Implemented CRUD operations using JDBC
* Designed relational database tables for hospital workflows
* Gained hands-on experience in backend development and database design

---

## 🎓 Education

**B.E. – Computer Science and Engineering**
Akshaya College of Engineering and Technology
📅 2021 – 2025 | **CGPA:** 8.24

**Higher Secondary Certificate (HSC)**
Sri Krishna Reddy Siddhartha Junior College
📅 2019 – 2021 | **Percentage:** 94.2%

**Secondary School Certificate (SSC)**
Zilla Parishad High School
📅 2018 – 2019 | **Percentage:** 98%

---

## 🏆 Certifications & Achievements

* 🏅 Dr. Kalam Young Achiever Award – World Youth Federation
* 💼 Software Engineering Job Simulation – JPMorgan Chase (Forage)
* 🌐 Network Essentials – Cisco (Naan Mudhalvan)
* 🎨 UI & UX Design – InGage
* 🤖 AI Tools & ChatGPT Workshop – be10x
* ☁️ MongoDB Atlas – MongoDB University
* ⚙️ Automation Fundamentals – UiPath
* 🖥️ ServiceNow System Administrator – FutureSkills Prime
* 📊 MS Office 365 Productivity Suite (Advanced)

---

## 📍 Contact Me

📍 Tirupati, Andhra Pradesh, India
📧 Email: **[peetlamahesh81@gmail.com](mailto:peetlamahesh81@gmail.com)**
📞 Phone: **+91 9182 353 829**

---

⭐ *Thank you for visiting my profile! Feel free to connect or collaborate.*
